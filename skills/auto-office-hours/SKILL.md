...
- **Deferred** to later work, with the reason, recorded in the skeleton's scope coverage. Promote to `ROADMAP.md` only when the user approves a phased decomposition.
...
- The approved skeleton states the objective in one sentence.
...
- The host/session can complete `SPEC.md` without dropping material request context.

If those conditions pass, load and follow `auto-frame`'s contract to complete `.agent/work/<change>/SPEC.md`, and let auto-frame record `canonical_spec`. If any condition fails, stop after the skeleton with the blocker or focused question.
...
Approved path:
- SPEC skeleton written to `.agent/work/<change>/SPEC.md` from `references/spec-skeleton.md`.
...
- Scope preservation records whether the skeleton preserves the full stated intent or intentionally decomposes it.
- Deferred scope is named in the skeleton; it is promoted to `ROADMAP.md` only on a user-approved phased decomposition.
...
- Approved, frame-ready skeleton continues inline into `auto-frame` without another user prompt.

The skeleton is a decision record, not a transcript. It is a faithful record of what the user approved, not the agent's editorial rewrite.
