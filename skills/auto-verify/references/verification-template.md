...
Two uses of one shape: the inline conversation report (always), and on PASS the same content written to `PLAN.md` as a terminal `## Verification` section (append-replace). The durable copy keeps the Summary block plus one rollup line per slice; it drops conversational framing and never stacks on a prior section.
