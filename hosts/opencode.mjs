...
export const opencodeHost = {
  ...
  toolMapping: {
    ...
    questions: 'Use the `question` tool. Pass `questions`, each with `question`, `header` (max 30 chars), `options` (`{label, description}` pairs, labels 1 to 5 words), and `multiple` when more than one answer is valid. Put your recommended option first and suffix its label with "(Recommended)". Do not add an "Other" option: the tool adds a custom-answer choice. The call blocks until the user answers or dismisses. The tool ships enabled for the CLI, TUI, and desktop clients; when it is not among your tools, ask in plain assistant text and end the turn.',
