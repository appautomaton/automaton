...
test('auto-verify treats pass as completed change, not resume handoff', () => {
  ...
  assert.match(skill, /New objective.*`auto-frame`/)
  ...
  assert.match(lifecycle, /`auto-frame` mention at `verified` is for a new objective, not a same-change handoff/)
...
test('auto-verify escalates a repeated criterion failure to auto-plan', () => {
  ...
  assert.match(skill, /\*\*Next:\*\* auto-plan, <reason naming the repeated criterion>/)
  assert.match(lifecycle, /`auto-plan` on a repeated fail of the same criterion/)
})
