...
// Every warning the CLI install/drift surface can emit, pinned once. The
// constructor throws on an unknown code, so the vocabulary enforces itself at
// emission time — the CLI-side analogue of PREREQUISITE_DIAGNOSTIC_CODES.
// Deliberately not in contracts-data.json: install warnings are not part of
// the runtime contract that ships into target projects.
export const WARNING_CODES = Object.freeze([
  'overwrote_existing_file',
  'overwrote_modified_file',
  'kept_modified_file',
  'removed_modified_file',
...
// Hosts install independently, so the global stamps alone cannot say which
// host's copies are stale or when they arrived; each owner records the
// version and time of its own last install. Optional and additive within
// schema 1: old receipts load with an empty map and drift falls back to the
// global stamp.
function normalizeOwners(parsed) {
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    return {}
  }
  return Object.fromEntries(
    Object.entries(parsed)
      .filter(([, record]) => typeof record?.automaton_version === 'string')
      .map(([owner, record]) => [owner, {
        automatonVersion: record.automaton_version,
...
export function saveReceipt(root, receipt) {
  ...
  const serialized = {
    ...
    owners: Object.fromEntries(
      Object.entries(receipt.owners).map(([owner, record]) => [owner, {
        automaton_version: record.automatonVersion,
...
export function mergeReceipt(previous, recorder, { automatonVersion, installedAt } = {}) {
  ...
  return {
    ...
    owners: {
      ...base.owners,
      ...(automatonVersion
        ? { [recorder.owner]: { automatonVersion, installedAt: installedAt ?? null } }
        : {})
    },
