...
function removeManifestOwnedStaleHostTools(root, host) {
  const manifest = loadManifest(root)
  const hostEntry = manifest.hosts[host.id]
  ...
  const keepPath = `${host.skillRoot}/auto-execute/references/HOST-TOOLS.md`
  let changed = false
  const retainedFiles = []

  for (const relativePath of hostEntry.files) {
    const isStaleHostTools = relativePath.startsWith(host.skillRoot + '/') &&
      relativePath.endsWith('/references/HOST-TOOLS.md') &&
...
export function installHost(host, { root = '.', sourceRoot }) {
  ...
  removeManifestOwnedStaleHostTools(paths.root, host)
