...
const CODEX_FEATURE_LINES = ['codex_hooks = true', 'multi_agent = true']
...
function injectSharedArtifacts(skillRoot, sharedRefsRoot, sharedScriptsRoot, options = {}) {
  const { root, managedPaths } = options
  const createdFiles = []

  // Inject shared references into each skill's references/ folder
  if (existsSync(sharedRefsRoot)) {
    for (const entry of readdirSync(sharedRefsRoot, { withFileTypes: true })) {
      if (!entry.isFile()) continue
      const skillRefsDir = join(skillRoot, 'references')
      mkdirSync(skillRefsDir, { recursive: true })
      const target = join(skillRefsDir, entry.name)
      const existed = existsSync(target)
      const shouldRefresh = root !== undefined && managedPaths?.has(relative(root, target))
      if (!existed || shouldRefresh) {
        cpSync(join(sharedRefsRoot, entry.name), target)
        if (!existed) {
  ...
  // Inject shared scripts into each skill's scripts/ folder
...
export function installHost(host, { root = '.', sourceRoot }) {
  ...
  // Inject shared references and scripts into each skill folder
  ...
  for (const entry of readdirSync(projectSkillRoot, { withFileTypes: true })) {
    ...
    createdFiles.push(...injectSharedArtifacts(skillPath, sharedRefsRoot, sharedScriptsRoot, {
      root: paths.root,
      managedPaths
    }))
  ...
  for (const [relativePath, content] of Object.entries(installFiles)) {
    ...
    if (relativePath === '.codex/config.toml') {
      ...
      if (result.injected) {
        mutations.codexHooksInjected = result.injectedFeatures.includes('codex_hooks = true')
        mutations.codexMultiAgentInjected = result.injectedFeatures.includes('multi_agent = true')
      }
      continue
    }
...
export function uninstallHost(host, { root = '.', sourceRoot }) {
  ...
  if (hostEntry.mutations?.codexHooksInjected || hostEntry.mutations?.codexMultiAgentInjected) {
    ...
    if (hostEntry.mutations?.codexMultiAgentInjected) {
      featureLines.push('multi_agent = true')
    }
