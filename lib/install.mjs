...
function renderHostToolsReference(host) {
  ...
  for (const role of SUBAGENT_ROLES) {
    const where = role.intent === 'explore'
      ? 'any stage; read-only one-shot lookup'
...
function ensureCodexHooksConfigured(target, desiredContent) {
  return ensureHookConfig(target, desiredContent, CODEX_HOOK_FRAGMENT_PATTERN)
}

function ensureClaudeHooksConfigured(target, desiredContent) {
  return ensureHookConfig(target, desiredContent, CLAUDE_HOOK_FRAGMENT_PATTERN)
}
...
function disableCodexHooks(target, fragments) {
  disableHookConfig(target, fragments, CODEX_HOOK_FRAGMENT_PATTERN)
}

function disableClaudeHooks(target, fragments) {
  disableHookConfig(target, fragments, CLAUDE_HOOK_FRAGMENT_PATTERN)
}
...
export function uninstallProject(root = '.') {
  ...
  // removed whole. Project history under `.agent/` (work, wiki, steering the
...
export function installHost(host, { root = '.', sourceRoot }) {
  ...
  for (const dispatchingSkill of ['auto-execute', 'auto-office-hours', 'auto-frame', 'auto-plan']) {
  ...
  for (const [relativePath, content] of Object.entries(installFiles)) {
    ...
    if (relativePath === '.codex/config.toml') {
      ...
      const previousLines = previousMerges.get(relativePath)?.addedLines ?? []
    ...
    if (relativePath === '.codex/hooks.json') {
      recorder.ensureDir(dirname(target))
      const { fragments } = ensureCodexHooksConfigured(target, content)
      pruneStaleMergeFragments(target, previousMerges.get(relativePath), fragments, disableCodexHooks)
    ...
    if (relativePath === '.claude/settings.json') {
      recorder.ensureDir(dirname(target))
      const { fragments } = ensureClaudeHooksConfigured(target, content)
      pruneStaleMergeFragments(target, previousMerges.get(relativePath), fragments, disableClaudeHooks)
      recorder.recordMerge(target, { fragments })
      continue
    }
...
function uninstallHostFromReceipt(host, paths, receipt, ownerEntries, warn) {
  ...
  for (const entry of ownerEntries.merges) {
    ...
    } else if (entry.path === '.codex/hooks.json') {
      disableCodexHooks(target, new Set(entry.fragments))
    } else if (entry.path === '.claude/settings.json') {
      disableClaudeHooks(target, new Set(entry.fragments))
    }
...
function uninstallHostLegacy(host, paths, { sourceRoot }) {
  ...
  for (const [relativePath, content] of Object.entries(installFiles)) {
    ...
    if (relativePath === '.codex/hooks.json') {
      disableCodexHooks(target, desiredHookFragments(content, CODEX_HOOK_FRAGMENT_PATTERN))
      pruneEmptyDirectories(dirname(target), join(paths.root, '.codex'))
      continue
    }

    if (relativePath === '.claude/settings.json') {
      disableClaudeHooks(target, desiredHookFragments(content, CLAUDE_HOOK_FRAGMENT_PATTERN))
