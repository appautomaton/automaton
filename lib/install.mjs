...
// during installHost(); uninstallHost() removes the same set. Add a role here, and
// every host adapter picks it up through host.renderAgentDefinition / agentRelativePath.
...
function readRoleBody(sourceRoot, roleId) {
  return readFileSync(
    join(sourceRoot, 'skills', 'auto-execute', 'references', `${roleId}-role.md`),
    'utf8'
  )
}
...
function renderHostToolsReference(host) {
  ...
  for (const role of SUBAGENT_ROLES) {
    lines.push(`- \`${role.agentName}\` — ${role.description}`)
  }
...
export function installHost(host, { root = '.', sourceRoot }) {
  ...
  // on reinstall; durable role authoring belongs in skills/auto-execute/references/*-role.md.
  if (typeof host.renderAgentDefinition === 'function' && typeof host.agentRelativePath === 'function') {
    for (const role of SUBAGENT_ROLES) {
      ...
      writeFile(target, host.renderAgentDefinition(role, roleBody))
    }
  }
