...
// Skill directories Automaton has shipped and retired. Append-only, for the same
// reason SUBAGENT_ROLES ids are (DD-008, DD-020): a newer installer cleans an older
// install only because this list is a superset of every earlier version's removals.
// Add a name here whenever a skill is retired; never delete one, or that skill is
// stranded forever in every project still carrying it. Pruning is by exact name, so
// a user-authored `auto-*` skill is never a candidate.
export const RETIRED_SKILLS = [
  'auto-onboard',      // DD-016: descriptive steering retired
  'auto-office-hours', // DD-017: folded into auto-frame
  'auto-ceo-review'    // pre-0.3: product review is the user reading SPEC.md
]
...
function renderHostToolsReference(host) {
  ...
  for (const role of SUBAGENT_ROLES) {
    const where = role.intent === 'explore'
      ? 'frame, plan, and execute; read-only one-shot lookup'
  ...
  lines.push(
    ...
    // The question tool is the one host capability every skill reaches for: FRAMEWORK's
    // Asking The User convention says to use it, and without a name per host that
    // instruction is a guess. Each host's entry carries the real tool, its option
    // schema, and its availability gate, because all three are conditionally exposed.
    `- questions: ${mapping.questions ?? 'No host-native question tool: ask in plain assistant text with the options enumerated, and end the turn.'}`,
...
// Remove skills this version retired. Every install runs this, receipt or not:
// the case that strands orphans is precisely a receipt that never recorded them
// (a pre-receipt install, or one written by an upgrade that no longer ships the
// files). Removal is loud, never silent, because a retired skill may have been
// edited locally and the warning is the only record the user gets.
function pruneRetiredSkills(targetRoot, context = null) {
  if (!existsSync(targetRoot)) {
    return
  }
  ...
  for (const skillName of RETIRED_SKILLS) {
    ...
    if (context) {
      ...
      context.recorder.warn(
        'pruned_retired_skill',
        `${rel ?? skillName} is a retired automaton skill and has been removed`
      )
    }
    rmSync(target, { recursive: true, force: true })
  }
...
function syncHostSkills(sourceRoot, targetRoot, context = null) {
  ...
  pruneRetiredSkills(targetRoot, context)
...
// recomputed from the current source tree, exactly as before DD-011, plus the
// retired names (a source recompute alone cannot see a skill the source dropped).
function uninstallHostLegacy(host, paths, { sourceRoot }) {
  ...
  pruneRetiredSkills(projectSkillRoot)
